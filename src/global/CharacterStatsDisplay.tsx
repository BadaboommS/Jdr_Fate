                        ...
                        {Object.entries(charStats.Caracteristics).map(([key, value]) => (
                            <div key={key} className={`${(key === "SPD")?'cursor-help ' : ''}input_entry`} title={(key === "SPD")? `Vitesse: ${charStats.CharSpeed} Km/h` : ''}>
                                ...
                                <span className={`input_field ${charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics]? charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics] < 0? "!text-red-500" : "!text-blue-500" : ""}`}>
                                    {value}
                                    {charStats.CaracteristicsOverload[key as keyof typeof charStats.CaracteristicsOverload] !== 0 && (charStats.CaracteristicsOverload[key as keyof typeof charStats.CaracteristicsOverload] > 0
                                        ? "+".repeat(charStats.CaracteristicsOverload[key as keyof typeof charStats.CaracteristicsOverload])
                                        : "-".repeat(Math.abs(charStats.CaracteristicsOverload[key as keyof typeof charStats.CaracteristicsOverload])))
                                    }
                                    {charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics] !== 0 && ` ${charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics] > 0
                                        ? `+${charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics]}`
                                        : charStats.CaracteristicsBuff[key as keyof typeof charStats.Caracteristics]}`
                                    }
                                </span>
                            </div>
                ...
                <div className="input_group min-w-[200px]">
                    ...
                    {charStats.BuffsList.map((buff, index) => (
                        <div key={index} className="input_entry">
                            ...
                            {handleRemoveEffect && <button onClick={() => handleRemoveEffect(charStats, buff, "Buff")} className="bg-red-900 text-white p-1 cursor-pointer"><RxCross1 size={20} /></button>}
                        </div>
                    ))}
                    ...
                    {charStats.DebuffsList.map((debuff, index) => (
                        <div key={index} className="input_entry">
                            ...
                            {handleRemoveEffect && <button onClick={() => handleRemoveEffect(charStats, debuff, "Debuff")} className="bg-red-900 text-white p-1 cursor-pointer"><RxCross1 size={20} /></button>}
                        </div>
                    ))}
                </div>
            </div>
